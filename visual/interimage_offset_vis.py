...
if __name__ == "__main__":
    ...
    with torch.no_grad():    
        for img_path, ann_path in bar:
            ...
            for inx, depths in enumerate(cfg.DEPTHS):
                for depth in range(depths):
                    hk = model.deformconvcrack.levels_down[inx].blocks[depth].dcn.offset.register_forward_hook(savehook)

            output = model(input_tensor)
            hk.remove()
            
            offset_outputs = GenerateOffsets(savehook)
            row, col = np.nonzero(ann)
            img_list  = [copy.deepcopy(rgb) for i in range(len(cfg.DEPTHS))]
            for loc_i in range(0, len(row), 20):
                plot_offsets(img=rgb, img_list=img_list, save_output=offset_outputs, roi_x=col[loc_i],roi_y=row[loc_i])    
