...
def bce2d(input, target):
    ...
    loss = F.binary_cross_entropy(log_p, target_trans.float(), weight, reduction=True)
    return loss

class cross_entropy_loss2d():

    def __init__(self) -> None:
        pass

    def __call__(self, inputs, targets, balance=1.1):
        """
        ...
        loss = nn.BCELoss(weights, reduction=False)(inputs, targets)
        return loss
def cross_entropy_loss_RCF(prediction, label):
    ...
    # loss = F.binary_cross_entropy(prediction, label2, mask, reduction=True)
