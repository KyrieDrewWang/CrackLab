...
class Visualizer(object):
    ...
    def save_settings(self, save_path="log/visdom", save_log=True, save_img=False, save_plot=False):

        # save_format = '{info}-{time}'.format(time=time.strftime("%Y-%m-%d"), info=self.env)  #%H:%M:%S
        # save_format = '{info}-{time}'.format(time=time.strftime("%Y-%m-%d%H:%M:%S"), info=self.env)  
        save_format = '{info}'.format(info=self.env)  
    ...
    def log(self, info, win='defalut'):
        ...
        if isinstance(info, dict):
            info = dict([(str(k), str(v)) for k,v in info.items()])
        if self.save_log:
            with open(os.path.join(self.save_path, '%s.txt'%win), 'a') as f:
                if isinstance(info, dict):
                    json_str = json.dumps(info, indent=0)
