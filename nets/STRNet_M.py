...
import sys
sys.path.append('.')
from config.config_STRNet_M import Config as cfg
import os
...
class Swish(nn.Module):
    def __init__(self):
        ...
        self.theta=torch.ones(1,requires_grad=True, device='cuda')
    def forward(self, x):
        x = x*torch.sigmoid(self.theta*x)
        return x
...
class STR_module1(nn.Module):
    def __init__(self, channel_in,A,B,fx,dwsize=3):
        ...
        self.block3=nn.Sequential(
            ...
            nn.GroupNorm(int(channel_in*A)//4,int(channel_in*A)),
            self.activate
        )
...
class STR_module3(nn.Module):
    def __init__(self, channel_in,A,B,fx,dwsize=3):
        ...
        self.block1=nn.Sequential(
            PWConv(channel_in,int(channel_in*B)),
            nn.GroupNorm(int(channel_in*B)//4,int(channel_in*B)),
            self.activate
        )
        ...
        self.pw_final=nn.Sequential(
            ...
            nn.GroupNorm(self.channel_out//4,self.channel_out)
        )
    def forward(self,x):
        residual=x
        x_1=x=self.block1(x)
        ...
        x_2=F.interpolate(x_2,scale_factor=2,mode='bilinear') 
...
class Attention_decoder(nn.Module):
    def __init__(self,in_channel):
        ...
        self.conv=nn.Sequential(
            ...
            nn.GroupNorm(in_channel//4,in_channel)
        )
        self.pw_Q=nn.Sequential(
            PWConv(in_channel,in_channel//2),nn.GroupNorm(in_channel//8,in_channel//2)
            )
...
class Coarse_Upsample(nn.Module):
    def __init__(self, channel_in,channel_out):
        ...
        self.norm=nn.GroupNorm(channel_out//4,channel_out)
        self.relu=nn.ReLU()
    def forward(self, x):
        x=self.upsample(x)
        x=self.conv(x)
...
class STR_module_full(nn.Module):
    def __init__(self, channel_in):
        ...
        self.STR5=STR_module3(channel_in,12,2.5,'swish',dwsize=5)
        self.STR6=STR_module3(channel_in,12,2.5,'swish',dwsize=5)
        ...
        self.STR9=STR_module2(channel_in,12,3,'swish',2,dwsize=5)
        self.STR10=STR_module3(channel_in,12,3,'swish',dwsize=5)
        self.STR11=STR_module3(channel_in,12,3,'swish',dwsize=5)
...
class STRNet_M(nn.Module):
    def __init__(self, 
                 channel_in=cfg.CHANNEL_IN,
                 channel_out=cfg.CHANNEL_OUT):
        super(STRNet_M, self).__init__()
        self.dim=48
        self.stem=nn.Sequential(
            Conv3_3(3,self.dim),
            nn.GroupNorm(self.dim//4,self.dim),
            HSwish()            
        )
    ...
    def forward(self, x):
        ...
        x_aux=F.interpolate(self.attention3(x_aux,x_aux),scale_factor=2, mode='bilinear')
