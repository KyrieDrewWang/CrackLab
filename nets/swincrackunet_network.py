...
class SwinCrackUnet(nn.Module):
    r""" Swin Transformer Unet architecture for crack detection 
    ...
    def __init__(self, 
        ...
        print("SwinTransformerSys expand initial----depths:{};drop_path_rate:{};num_classes:{}".format(
        ...
        for i_layer in range(self.num_layers):
            # todo: attention gating mechanism instead of concat_linear
    ...
    def forward_down_features(self, x):
        x, H, W = self.patch_embed(x)   # torch.Size([1, 16384, embedded dim])
        ...
        for layer_down in self.layers_down:
            x, H, W, xx = layer_down(x, H, W)
            x_downsample.append(xx)  # todo: the residual connection from where ??
    ...
    # final upsample to the resolution of the input
